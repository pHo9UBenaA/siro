...
caller's package-manager version. `RuleBinding.versionNote` is the canonical runtime metadata for
the `(available since …; default safe since …)` suffixes. This table is its cited research ledger;
a sync test keeps every runtime version literal represented here.
...
> Last verified: 2026-09-04. Bolded cells are confirmed against upstream changelogs/release
...
| aube | `jailBuilds`, `strictDepBuilds` (aube-workspace.yaml) | TBD (no versioned changelog found)                                                                                                                                 | n/a (both must be enabled explicitly) ([docs](https://aube.en.dev/package-manager/configuration.html))                                                                                                   | `jailBuilds` sandboxes scripts; `strictDepBuilds` requires an explicit allow/deny decision.                                                                                                         |
...
| deno | `lock.frozen` (deno.json)                    | TBD (the related `--frozen-lockfile` CLI flag shipped in **deno 1.45.0** — [release](https://github.com/denoland/deno/releases/tag/v1.45.0))          | n/a                                                                                                                                  | The binding reads the configuration-file form; its introduction version remains unverified.                          |
...
| deno | `minimumDependencyAge` (deno.json)        | TBD (introduced in Deno 2.x) ([docs](https://docs.deno.com/runtime/reference/deno_json/))                                       | **deno 2.9.0** (1440 minutes) ([release](https://github.com/denoland/deno/releases/tag/v2.9.0))                                                                             | ISO-8601 duration, date, timestamp, minutes number, or object; object `age` is optional since **deno 2.9.4** ([release](https://github.com/denoland/deno/releases/tag/v2.9.4)). |
...
| PM   | Key                                                        | Available since                                                                                                   | Default safe since | Notes                                                     |
| ---- | ---------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------- | ------------------ | --------------------------------------------------------- |
| npm  | `provenance=true` (.npmrc)                                 | **npm 9.5.0** ([release](https://github.com/npm/cli/releases/tag/v9.5.0))                                         | n/a                | Requires OIDC-capable CI.                                 |
...
| bun  | `provenance=true` (.npmrc; publish via `bunx npm publish`) | n/a (native `bun publish` support is tracked in [oven-sh/bun#15601](https://github.com/oven-sh/bun/issues/15601)) | n/a                | The binding requires the npm-compatible publication path. |
...
| PM   | Key                                   | Available since | Default safe since | Notes                                               |
| ---- | ------------------------------------- | --------------- | ------------------ | --------------------------------------------------- |
| npm  | `publishConfig.access` (package.json) | TBD             | n/a                | Accepts an explicit `public` or `restricted` value. |
| pnpm | `publishConfig.access` (package.json) | TBD             | n/a                | Uses the shared package manifest.                   |
| yarn | `publishConfig.access` (package.json) | TBD             | n/a                | Uses the shared package manifest.                   |
| bun  | `publishConfig.access` (package.json) | TBD             | n/a                | Uses the shared package manifest.                   |
| aube | `publishConfig.access` (package.json) | TBD             | n/a                | Uses the shared package manifest.                   |

## `files-field`

| PM   | Field                         | Available since | Default safe since | Notes                                     |
| ---- | ----------------------------- | --------------- | ------------------ | ----------------------------------------- |
| npm  | `files` (package.json)        | always          | n/a                | Shared package-manifest binding.          |
| pnpm | `files` (package.json)        | always          | n/a                | Shared package-manifest binding.          |
| yarn | `files` (package.json)        | always          | n/a                | Shared package-manifest binding.          |
| bun  | `files` (package.json)        | always          | n/a                | Shared package-manifest binding.          |
| aube | `files` (package.json)        | always          | n/a                | Shared package-manifest binding.          |
| deno | `publish.include` (deno.json) | TBD             | n/a                | Applies only to publishable JSR packages. |
...
| aube | `blockExoticSubdeps` (aube-workspace.yaml) | TBD (no versioned changelog found)                                                                               | TBD                                                                                                        | Aube exposes the same policy in its workspace configuration.                                             |
...
| aube | `trustPolicy` (aube-workspace.yaml) | TBD (no versioned changelog found)                                               | n/a                 | Set to `no-downgrade` to reject trust-level decreases.                                        |
